...
- [x] 创建项目会先收集用户背景和代码经验，方便 AI 调整访谈深度。
- [x] 访谈问题支持填写自己的想法，并可标记“后续再考虑”。
- [x] ProjectSpec 生成按钮增加最低对齐度门槛，避免首次访谈后误生成过薄报告。
- [x] Markdown 生成请求失败时会降级使用本地模板，不再卡死在本地代理错误。
- [x] ProjectSpec 页面底部生成按钮与底部导航不再重叠。
